...
Git.prototype.fetchLatest = function(cb) {
  utils.log.info('fetching latest commits');
  var repoDir = this._repoDir;
  exec('git pull', { cwd: repoDir }, function(err) {
    if(err) utils.errorAndExit('could not fetch latest version of repo');
    cb();
  });
};
...
Git.prototype.cloneRepo = function(cb) {
  ...
  utils.log.info('cloning repo ' + src + ' to ' + repoDir);
...
Git.prototype.checkout = function(cb) {
  ...
  utils.log.info('checking out ' + branch);
  exec('git checkout ' + branch, { cwd: this._repoDir }, function(err) {
    if(err) utils.errorAndExit('failed to checkout ' + branch);
    cb(null);
  });
};
